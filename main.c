...
int main() {
    ...
    while (1) {
        ...
        temperature = temperature / 100.0f; // Temperatura em centésimos
       ...
       // Cria uma matriz de bytes para o payload
        uint8_t payload[12]; // 3 floats * 4 bytes/float = 12 bytes
        uint8_t payload_len = sizeof(payload);
        
        float_to_byte temp_bmp_conv, temp_aht_conv, hum_aht_conv;

        // Atribui os valores dos sensores aos membros float da union
        temp_bmp_conv.f = (float)temperature / 100.0f;
        temp_aht_conv.f = data.temperature;
        hum_aht_conv.f = data.humidity;
        
        // Copia os bytes de cada float para o payload
        memcpy(&payload[0], temp_bmp_conv.b, 4);
        memcpy(&payload[4], temp_aht_conv.b, 4);
        memcpy(&payload[8], hum_aht_conv.b, 4);

        // Agora, o array `payload` contém os dados corretamente serializados
        lora_send_packet(payload, payload_len);
    }
